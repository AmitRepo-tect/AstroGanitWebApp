...
export class FestivalComponent {
  ...
  addObservers() {
    ...
    this.panchangService.festData$.subscribe(data => {
      if (data != null) {
        //this.festDetailResponse = data;
        //console.log(data)
        this.festivalsByMonth = data.festDetail;
      } else {
        this.fetchFestData();
      }
    });
  ...
  fetchFestData(): void {
    this.panchangService.getFestData(this.panchangInput?.place.Place!, this.panchangInput?.place.state!, this.panchangInput?.place.latitude!, this.panchangInput?.place.longitude!, this.panchangInput?.place.timezone!,
      this.panchangInput?.date.getDate()!, this.panchangInput?.date.getMonth()!, this.panchangInput?.date.getFullYear()!).subscribe(
        data => {
          this.panchangService.setFestData(data);
        },
        error => {
