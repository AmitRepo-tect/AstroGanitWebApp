...
@Component({
  selector: 'app-rahukaal',
  templateUrl: './rahukaal.component.html',
  styleUrl: './rahukaal.component.scss'
})
export class RahukaalComponent {
  ...
  addObservers() {
    ...
    this.panchangService.panchangData$.subscribe(data => {
      if (data != null) {
        ...
        console.log(data.doGhatiResponse.doGhatiList)
      } else {
  ...
  getDayFromDate(date: string): string {
    const dateString = '06 May, 2025';
    const parsedDate = new Date(date);

    // Get the day of the week (0 = Sunday, 1 = Monday, ..., 6 = Saturday)
    const dayIndex = parsedDate.getDay();

    // Optional: Get day name from array
